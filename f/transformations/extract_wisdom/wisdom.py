"""
EXTRACT_WISDOM - Извлечение инсайтов по методу Daniel Miessler

ЧТО ДЕЛАЕТ:
- Извлекает SUMMARY, IDEAS, INSIGHTS, QUOTES, HABITS, FACTS, REFERENCES, RECOMMENDATIONS
- Анализирует контент на основе методологии Daniel Miessler

ГДЕ ИСПОЛЬЗУЕТСЯ:
- Используется для глубокого анализа статей, книг, видео
...
def main(
    ...
    """
    Извлекает инсайты и структурированную информацию из текста.

    @param input Текст для анализа
    @param model Модель для использования
    @return Словарь с полем output - структурированный анализ
    """
